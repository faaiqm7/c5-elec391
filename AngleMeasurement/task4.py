...
class GyroComplementaryFilterPlotter:
    def __init__(self, port="COM3", baud_rate=9600):
        self.ser = serial.Serial(port, baud_rate, timeout=1)
        self.qData, self.tData = [], []
        self.fig, self.ax = plt.subplots(figsize=(10, 6))
        self.qLine, = self.ax.plot([], [], label="Filtered Angle (QFinal)", color="purple")
        self.qText = self.ax.text(0.02, 0.95, "", transform=self.ax.transAxes, fontsize=10, verticalalignment='top')
        self._setup_axes()

    def _setup_axes(self):
        self.ax.set_title("Filtered Angle (QFinal) over Time")
    ...
    def update(self, frame):
        ...
        if qValue is not None:
            ...
            self.qText.set_text(f"QFinal: {qValue:.4f}°")
...
if __name__ == "__main__":
    plotter = GyroComplementaryFilterPlotter()
    plotter.run()
